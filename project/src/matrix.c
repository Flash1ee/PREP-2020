...
Matrix* create_matrix(size_t rows, size_t cols) {
    ...
    matrix->data = calloc(rows, sizeof(matrix_t*));
    if (!matrix->data) {
        free(matrix);
    ...
    for (size_t i = 0; i < rows; i++) {
        matrix->data[i] = calloc(cols, sizeof(matrix_t));
...
Matrix* create_matrix_from_file(const char* path_file) {
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < cols; j++) {
            if (fscanf(in, ScMyMatrix, &(matrix->data[i][j])) != 1) {
                fclose(in);
                free_matrix(matrix);
...
Matrix* sub(const Matrix* lhs, const Matrix* rhs) {
    if (!lhs || !rhs || !rhs->data || !lhs->data) {
        return NULL;
    }
    if (lhs->rows != rhs->rows || lhs->cols != rhs->cols) {
        return NULL;
    }
    if (rhs->rows < 1 || rhs->cols < 1 || lhs->rows < 1 || lhs->cols < 1) {
        return NULL;
    }
...
static void get_minor(const Matrix* src, Matrix* dst, size_t cur_col, size_t cur_row) {
    size_t dst_rows = 0;
    size_t dst_cols = 0;
